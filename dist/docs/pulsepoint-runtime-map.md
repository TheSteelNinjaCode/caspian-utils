...
- `.venv/Lib/site-packages/casp/components_compiler.py` injects `pp-component` after component expansion, frames a multi-root component with the fragment comment pair, and supplies a `display: contents` boundary host for multi-root pages/layouts and for composition components.
...
| --- | --- | --- | --- |
| Component roots | page templates in `src/app/**/index.py`, layout templates in `layout.py`, component `html(...)` markup | `components_compiler.py`, `public/js/pp-reactive-v2.min.js` | one authored root, final expanded root receives one `pp-component`, no sibling scripts. A **component** that is multi-rooted raises `TemplateRootError`; a `.py` **page or layout** does not — see the next row |
| Boundary host (multi-root pages/layouts, composition components) | sibling top-level nodes in a `.py` `index.py`/`layout.py`; a component whose authored root is another `x-*` tag | `components_compiler.py` (`allow_fragment_host`, `_wrap_with_pp_component_host`), `public/js/pp-reactive-v2.min.js` | the compiler emits `<div pp-component="…" style="display: contents">` around the template instead of raising; it is layout-neutral but real in the DOM, carries the boundary identity and the owned script's scope, and in the composition case receives the parent's forwarded `x-*` props plus `pp-ref-forward`. Not author-written. See [pulsepoint.md](./pulsepoint.md) "Multi-root pages and layouts" |
| Range boundary (`<pp-fragment>`) | a component template with sibling top-level nodes | `public/js/pp-reactive-v2.min.js` | the compiler frames a multi-root component with a `<!--pp:id-->…<!--/pp-->` comment pair; the runtime materializes it into `<pp-fragment style="display: contents">` at mount and on SPA navigation, skipping pairs under `table`/`tbody`/`tr`/`select`/`optgroup` where a wrapper would be foster-parented out (the grouping still renders there, but the fragment owns no identity). The markers are compiler output — never hand-write them. A fragment has no root element, so it cannot receive props |
...
| Slot-authored scripts (composition roots) | a page, layout, or component whose authored root is an `x-*` tag, with the `<script>` inside it | `components_compiler.py`, `public/js/pp-reactive-v2.min.js` | the script travels as slot content wrapped in `<template pp-owner="…">`. The runtime resolves the owner — an exact `pp-component` id, or the alias `app`, which resolves to the nearest page boundary (root boundary for a layout) — and the owning template recovers and executes the script in its own scope; the script is not rendered into the child's DOM. If a script seems not to run, check the `pp-owner` value in the **served** HTML, not the post-mount DOM. See [pulsepoint.md](./pulsepoint.md) "A slot-authored `<script>` belongs to the template that authored it" |
| **Handler fires but the function is undefined** (`[PP-ERROR] Handler failed: ReferenceError: <fn> is not defined`) | an `on*` attribute on markup passed as children into another component | `components_compiler.py`, `public/js/pp-reactive-v2.min.js` | the handler resolves in the scope of the template that *authored* the markup, not the boundary the element sits in, so the function must be declared by that template's owned script. Wrapping the element in its own component does **not** move the owner; portaling is not the cause. See [pulsepoint.md](./pulsepoint.md) "A handler in slot content runs in the authoring template's scope" |
...
| Prop bag unavailable in nested markup or slots | `open="{!!pp.props.open}"`, `hidden="{!!pp.props.rotate}"`, `project-id="{pp.props.projectId}"` failures | The authoring component script; `resolveBoundaryAttributeValue`, `evalPropExpression`, `renderOwnedContent` in `public/js/pp-reactive-v2.min.js` | Read `pp.props` in the owning script and expose top-level names for markup; root forwarding alone does not supply the script API to every expression scope. See [script-to-template prop bindings](./pulsepoint.md#read-props-in-the-script-bind-names-in-markup) and [frontend verification](./testing.md#frontend-verification-for-agents) |
...
| RPC | `pp.rpc(...)` in scripts, `@rpc()` in Python | `public/js/pp-reactive-v2.min.js`, `casp/rpc.py` | `pp.rpc` is the only browser RPC entrypoint; options include abort, URL/CSRF URL, credentials, streaming, and upload callbacks; protected actions use `@rpc(require_auth=True)` |
...
| Navigation loading UI | `src/app/**/loading.py` exporting `def loading():`, plus `pp-loading-content="true"` on the pane it replaces | `casp/loading.py`, `casp/caspian_config.py`, `casp/layout.py`, `public/js/pp-reactive-v2.min.js` | optional per subtree — no loader means a plain fade, which is the common case. When one is wanted, **it is a shipped file convention: do not hand-roll a navigation spinner or a `pp:navigation:start` listener.** `loading()` is synchronous and parameterless; the closest ancestor URL scope wins (dynamic `[id]` folders never match); the markup is injected as raw HTML and never mounted, so `<x-*>` tags and `{ }` bindings do not resolve; `pp-loading-url` is runtime-emitted. In-page waits stay in component state. See [file-conventions.md](./file-conventions.md#loadingpy) |
...
- Read [file-conventions.md](./file-conventions.md) before building a navigation loading state, subtree shell, 404, or 500 page. Each is an optional file convention (`loading.py`, `layout.py`, `not_found.py`, `error.py`): add one only when the task asks for that behavior, and when it does, the question is which file to add, not what to build.
...
- Author one root element or one imported `x-*` root per route, layout, or component template. A component with siblings becomes a props-less fragment; a page or layout with siblings gets a `display: contents` boundary host. Neither raises — only a component with no root at all, or with an unresolvable `x-*` tag as its only root, raises `TemplateRootError`.
- Do not hand-write `<pp-fragment>` or `<!--pp:…-->` range markers. Fragment syntax is implicit — write sibling top-level nodes in the component template and the compiler emits the markers.
- Use lowercase `defaultvalue="{…}"` / `defaultchecked="{…}"` for an uncontrolled field, and `value="{state}"` / `checked="{state}"` for a controlled one — pick one per element for its lifetime or the runtime logs a `[PP-WARN]` mode-switch warning.
...
- "A page's `index.py` returns a header, a table and a footer as three sibling top-level nodes. Is that a render failure?" (Correct answer: no — the compiler wraps them in a `display: contents` boundary host. The same markup inside a *component* is a fragment, framed by a `<!--pp:id-->…<!--/pp-->` comment pair instead of a host element.)
- "Rendered DOM shows an extra `<div style="display: contents">` wrapping a component root. Where did it come from and should the template change?" (Correct answer: the composition host, carrying forwarded props and `pp-ref-forward`; expected output, do not author around it.)
- "Write a fragment component that returns two sibling `<tr>` rows." (Correct output writes the two `<tr>` elements as plain siblings in the component template and hand-writes no marker: the compiler emits the comment pair. It also does not pass props on the `<x-*>` tag — a fragment has no root to carry them and the compiler raises `FragmentPropsError`.)
