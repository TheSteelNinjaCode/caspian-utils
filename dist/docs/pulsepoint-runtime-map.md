...
| **Component script never ran** (silently no functions, no state, no effects, no listeners) | a page, layout, or component whose authored root is an `x-*` tag, with the `<script>` inside it | `components_compiler.py`, `public/js/pp-reactive-v2.min.js` | the script became the child's slot content, wrapped in `<template pp-owner="…">`. A boundary claims a slot script only when that owner value equals its own `pp-component`; a component's composition host matches, a page or layout (owned by `app`) never does. Check the **served** HTML, not the post-mount DOM. Fix by giving the template a native root. See [pulsepoint.md](./pulsepoint.md) "A template whose root is an `x-*` tag can lose its `<script>`" |
| **Handler fires but the function is undefined** (`[PP-ERROR] Handler failed: ReferenceError: <fn> is not defined`) | an `on*` attribute on markup passed as children into another component | `components_compiler.py`, `public/js/pp-reactive-v2.min.js` | the handler resolves in the scope of the template that *authored* the markup, not the boundary the element sits in, so the function must be declared by that template's owned script. Wrapping the element in its own component does **not** move the owner; portaling is not the cause. Frequently a downstream symptom of the row above. See [pulsepoint.md](./pulsepoint.md) "A handler in slot content runs in the authoring template's scope" |
