...
- Author one root element or one imported `x-*` root per route, layout, or component template. A component must have exactly one; a `.py` page or layout may have siblings and gets a boundary host.
