...
- `.venv/Lib/site-packages/casp/components_compiler.py` injects `pp-component` after component expansion, validates the single-root contract for components, and supplies a `display: contents` boundary host for multi-root `.py` pages/layouts and for composition components.
