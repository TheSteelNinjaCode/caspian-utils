...
| Range boundary (`<pp-fragment>`) | **none — not author-facing** | `public/js/pp-reactive-v2.min.js` | the runtime materializes a `<!--pp:id-->…<!--/pp-->` comment pair into `<pp-fragment style="display: contents">` at mount and on SPA navigation, skipping pairs under `table`/`tbody`/`tr`/`select`/`optgroup` where a wrapper would be foster-parented out. The browser half is shipped, but **no Caspian render path emits the markers yet** — never hand-write them, and never document this as an authoring feature |
...
- Do not hand-write `<pp-fragment>` or `<!--pp:…-->` range markers. There is no author-facing fragment syntax.
...
- "A page's `index.py` returns a header, a table and a footer as three sibling top-level nodes. Is that a render failure?" (Correct answer: no — the compiler wraps them in a `display: contents` boundary host. The same markup inside a *component* would raise `TemplateRootError`.)
...
- "Write a fragment component that returns two sibling `<tr>` rows." (Correct output refuses the fragment framing: there is no author-facing fragment syntax and no `<pp-fragment>`/`<!--pp:…-->` to hand-write; use one root, or `pp-for`.)
