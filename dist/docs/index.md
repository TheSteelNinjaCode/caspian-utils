...
- **Authoring is Python-only.** A route is one `index.py`, a layout one `layout.py`, a component one `.py` — each returning its markup from `html(r"""...""")`. There are no `.html` template files and no same-name HTML sidecars.
- Treat every authored route, layout, and component template like a React component return value **in shape only**: normally one top-level parent HTML element or one imported `x-*` root, with any owned plain `<script>` kept inside that same root. A component with sibling roots becomes a props-less fragment; a page or layout with sibling roots gets a compiler-supplied `display: contents` boundary host — see [pulsepoint.md](./pulsepoint.md) "Multi-root pages and layouts".
- A page, layout, or component whose root is an `x-*` tag keeps its `<script>` inside that root: the script travels as slot content and the runtime resolves its `pp-owner` (the alias `app` for a page or layout) back to the authoring template, which executes it in its own scope. A handler is likewise evaluated in the scope of the template that *authored* the markup, not the component the element ends up inside. See [pulsepoint.md](./pulsepoint.md) "A slot-authored `<script>` belongs to the template that authored it" and "A handler in slot content runs in the authoring template's scope".
...
- **Styling is one authored file and one generated file.** Write CSS in `src/app/globals.css`; it compiles to `public/css/styles.css`, which is the stylesheet the root layout links. That holds in every project — `caspian.config.json`'s `tailwindcss` flag only decides whether Tailwind is in the PostCSS plugin list, never where CSS lives. Never edit `public/css/styles.css`, and do not add a second hand-maintained stylesheet because Tailwind is disabled. See [project-structure.md](./project-structure.md).
- When `caspian.config.json` has `tailwindcss: true`, use Python `merge_classes(...)` plus browser `twMerge(...)` as the only supported Tailwind class-merging path, and author `globals.css` in Tailwind's CSS-first dialect (`@import "tailwindcss"`, `@theme`, `@source`).
- Validate app-owned Python with the single gate command `npm run test` (type check + lint + tests in one pass; auto-fix companion `npm run test:fix`). There is no separate `lint`, `typecheck`, or `check` script, and a bare `pytest` run is not the gate. See [testing.md](./testing.md).
- When an AI agent edits against an already-running dev stack, use the project's coordinated hold when present: complete the edit batch, then release once before frontend verification so the run costs one Python restart and one browser reload. See [agent-development.md](./agent-development.md).
...
- `file-conventions.md` - quick decision guide for `index.py`, `layout.py`, `loading.py`, `not_found.py`, and `error.py`, plus the owning runtime files to verify. All but `index.py` are optional; read this before hand-building a subtree shell, a navigation loading state, a 404, or a 500 page that the app has actually asked for
...
- `project-structure.md` - Default Caspian layout and where route files, reusable UI in `src/components/`, reusable non-UI code in `src/lib/`, database files, and the styling pair (`src/app/globals.css` authored, `public/css/styles.css` generated) belong
...
- `testing.md` - Recommended app-owned testing, type-checking, and linting gate over `main.py` and `src/**` (pyright + ruff + pytest behind one command, `npm run test`, so the gate and Pylance in the editor report the same thing), plus the auto-fix companion `npm run test:fix` and why ruff must not auto-delete component imports used as `<x-*>` tags (F401 unfixable); not a shipped feature and not gated by a `caspian.config.json` flag, so verify the actual command, tools, and config in the project
- `agent-development.md` - App-owned agent editing workflow for batching a multi-file run into one Python restart and one browser reload, including Claude Code, GitHub Copilot, and Codex hook configs, manual controls, expiry behavior, and frontend verification order
...
3. Before authoring or editing any `src/app/**` or component template, apply the root-shape rule: one authored root by default, any owned plain `<script>` inside that root, and no handwritten `pp-component`. A component with sibling roots becomes a props-less fragment; a page or layout with sibling roots gets a boundary host.
...
15. If the task is about tests, type checking, linting, a quality gate, CI checks, or making an app production-ready, read `testing.md`. The gate is `npm run test` (auto-fix companion: `npm run test:fix`) — one command covering type check, lint, and tests, with no separate `lint`/`typecheck`/`check` scripts beside it. It is an app-owned convention (not a shipped feature and not gated by a `caspian.config.json` flag), so confirm the script in `package.json`, the orchestrator in `settings/`, and the tools and config in `pyproject.toml` before assuming they exist.
16. If an AI agent is editing while the development stack is running, read `agent-development.md`, confirm the hold scripts and current host's hook config exist, finish the complete edit batch, and run `npm run dev:resume` once before browser verification.
17. If the task is about styling, a stylesheet, CSS variables, a theme, dark mode, or Tailwind configuration, edit `src/app/globals.css` and let it compile to `public/css/styles.css`. Read [project-structure.md](./project-structure.md) for the pipeline and [commands.md](./commands.md) for `npm run css` / `npm run css:build`. Check `caspian.config.json` and `postcss.config.js` to see whether Tailwind is one of the plugins before writing Tailwind-specific at-rules; when it is disabled, plain CSS in the same file is the answer, not a new stylesheet.
18. If the task is about static export, static-site generation, `output: export`-style output, previewing a static build over HTTP, pre-rendering dynamic routes, or a preview server whose port is already in use, read `static-export.md`. It is also an app-owned convention (not a shipped feature, not gated by a `caspian.config.json` flag), so confirm `settings/build-static.py`, `settings/serve-static.py`, and the `static`/`static:serve` scripts exist in the project before assuming static export is available.
