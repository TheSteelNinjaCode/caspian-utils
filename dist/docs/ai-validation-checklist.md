...
- AI edits `public/css/styles.css` directly, or adds a second stylesheet or an inline `<style>` block, instead of authoring `src/app/globals.css`. The compiled file is overwritten on the next build, so the change silently disappears. The `tailwindcss` flag is never a reason to bypass the pipeline — it only decides the PostCSS plugin list.
- AI validates a Python change by running `pytest` (or `ruff`, or `pyright`) alone and reports it as passing. The gate is `npm run test`, and any single tool is a subset of it.
