...
- Middleware is added in source order as `RPCMiddleware`, `AuthMiddleware`, `CSRFMiddleware`, `SessionMiddleware`, `BodySizeLimitMiddleware`, `MissingPublicAssetMiddleware`, `RateLimitMiddleware`, `PublicFilesMiddleware`, `SecurityHeadersMiddleware`, plus `RequestDiagnosticsMiddleware` outside production. The effective production request order is reversed: security headers, public files, rate limit, missing-public-asset, body limit, session, CSRF, auth, then RPC and routing. Development diagnostics are outermost. Existing public-file `GET`/`HEAD` requests stop at `PublicFilesMiddleware`. A **miss** whose first path segment is a real `public/` directory is then 404'd by `MissingPublicAssetMiddleware` — inside the rate limiter, outside sessions/CSRF/auth — so a bad asset URL cannot answer with a sign-in redirect or the sign-in page as `200 text/html`. Every other miss falls through to routing. Verify the current list in `main.py` rather than trusting an older order.
...
- The default CSP permits remote stylesheets (including CSS `@import`) and font files from any HTTPS origin through `style-src` and `font-src`. These defaults are provider-independent, since stylesheets and their font files can come from different CDNs. They grant broader loading access than a list of trusted hosts; applications can restrict origins using `CONTENT_SECURITY_POLICY`, which replaces the entire policy. Cross-origin font responses must still satisfy the browser's CORS requirements.
...
| Component imports and `x-*` tags | `src/components/**`, `src/app/**/*.py` | `casp.components_compiler`, `casp.component_decorator` | [components.md](./components.md), [routing.md](./routing.md) |
